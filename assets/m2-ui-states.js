jQuery(document).ready(function ($) {
  ...
  const BASE_IMAGES = {
    'Power Base': 'https://cdn.shopify.com/s/files/1/0152/7916/1444/files/adjustable-base.png',
    'HD Riser': 'https://cdn.shopify.com/s/files/1/0152/7916/1444/files/hd-riser-base.png',
    'Wooden Platform': 'https://cdn.shopify.com/s/files/1/0152/7916/1444/files/wooden-base.png?v=1750882521'
  };
  ...
  function setupEventListeners() {
    ...
    $(document).on('change', 'input[name="properties[Base Options]"]', function () {
      ...
      // Normalize label to match BASE_IMAGES keys
      const normalizedBase =
        selectedBedBase === 'Wood Platform' ? 'Wooden Platform' : selectedBedBase;

      const newSrc = BASE_IMAGES[normalizedBase];
      if (newSrc) {
        $('.baseOption img').attr('src', newSrc);
        console.log('[UI] Base image updated to:', newSrc);
      } else {
        console.warn('[UI] No base image found for:', selectedBedBase);
      }
    });
  ...
  function initializeToggles() {
    ...
    setTimeout(() => {
      $('.mattress2Options .bold_option_checkbox').each(function () {
        ...
        if (!$(this).find('.customToggleSwitch').length) {
